...
def get_files():
    for filepath,dirnames,filenames in os.walk(r'/WdHeDisk/users/zhangnong/MJO/use_s2s_data/grib_to_nc/remap_babj/OLR/'):
...
def read_data(filename):
    path='/WdHeDisk/users/zhangnong/MJO/use_s2s_data/grib_to_nc/remap_babj/OLR/'
    new_path='/WdHeDisk/users/zhangnong/MJO/711_test/s2s_data/babj/OLR/'
    ...
    first_time=time[0]-24
    trans_time=np.insert(time,0,[first_time])
    print("time shape:",trans_time.shape)
    #-------------------------------
    #这个要插入再分析数据的第一个
    location=np.argwhere(re_data_time==trans_time[0])
    in_data=re_data_olr[location,:,:]
    in_data=np.squeeze(in_data)
    insert_data=in_data
    print("insert data shape:",insert_data.shape)

    data_ttr=np.array(data.variables['ttr'][:,:,:,:])
    #累加，前去前一天的
    ...
    #构造新的nc文件u200
    da_olr=netCDF4.Dataset(new_path+filename[8:16]+'_babj_olr.nc','w',format='NETCDF4')
    da_olr.createDimension('latitude', 13)  # 创建坐标点
    da_olr.createDimension('longitude', 144)
    da_olr.createDimension('time',60)
    #设置变量
    ...
    da_olr.variables['time'][:]=trans_time[0:60]
    #设置u200变量
    olr =da_olr.createVariable('olr','f',('time','latitude','longitude'))
    olr.units = 'W m**-2'
    #填充数据
    da_olr.variables['olr'][:,:,:]=fina_data_ttr[0:60,:,:]
    da_olr.close()
