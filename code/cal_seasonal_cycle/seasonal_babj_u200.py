...
def find_229_location(date_time):
    ...
    return date_hour
#获取文件名
filenames=np.loadtxt('/WdHeDisk/users/zhangnong/MJO/711_test/2006_babj_date_sort.txt')
print('------')
print(filenames[0])
print(len(filenames))
#封装函数
def cal_seasonal_cycle(filename):
    path='/WdHeDisk/users/zhangnong/MJO/711_test/add_120_re_data/babj/U200/'
    #提取文件名的关键字
    date_time=int(filename)
    date_time_fina=date_time+160000
    #print(date_time)
    #print(date_time_fina)
    date_yearmonth=date_time//100
    date_month=date_yearmonth%100
    #date_year=date_time//10000
    run_year=np.array([2008,2012,2016,2020])
    #1～6月需要考虑2.29情况
    if date_month < 7:
        ...
        for i in range(date_time,date_time_fina,10000):
            new_filename=str(i)
            date_year=i//10000
            data=netCDF4.Dataset(path+new_filename+'_add_120redata_babj_u200.nc')
            data_olr=np.array(data.variables['u200'])
            data_time=np.array(data.variables['time'])
            #多个样本，平均一下
            print("mean data:",data_olr.shape)
            #删除1的维度，应该变（180，13，144）
        ...
        print(mean_data_2[176,9,7])
        #写入文件，前6个月有两个版本，方便后面rmm计算的时候减
        save_filename=str(filename)
        save_filename=save_filename[4:8]
        np.save('/WdHeDisk/users/zhangnong/MJO/711_test/seasonal_cycle/babj/U200/'+save_filename+'_seasonal_cycle_u200.npy', mean_data_1)
        print("save 1 success!!!")
        np.save('/WdHeDisk/users/zhangnong/MJO/711_test/seasonal_cycle/babj/U200/'+save_filename+'_run_seasonal_cycle_u200.npy', mean_data_2)
        print("save 2 success!!!")
    else:
        #符合babj的7，8，9，10，11，12月情况，不包含二月
        data_16year=[]
        for i in range(date_time,date_time_fina,10000):
            new_filename=str(i)
            data=netCDF4.Dataset(path+new_filename+'_add_120redata_babj_u200.nc')
        ...
        save_filename=str(filename)
        save_filename=save_filename[4:8]
        np.save('/WdHeDisk/users/zhangnong/MJO/711_test/seasonal_cycle/babj/U200/'+save_filename+'_seasonal_cycle_u200.npy', mean_data)
