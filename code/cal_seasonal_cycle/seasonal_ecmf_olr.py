...
def cal_seasonal_cycle(filename):
    path='/WdHeDisk/users/zhangnong/MJO/711_test/add_120_re_data/ecmf/OLR/'
    #提取文件名的关键字
    ...
    if (1 < date_month < 7)|(date_monthday==121):
        ...
        for i in range(date_time,date_time_fina,10000):
            new_filename=str(i)
            date_year=i//10000
            data=netCDF4.Dataset(path+new_filename+'_add_120redata_ecmf_olr.nc')
        ...
        np.save('/WdHeDisk/users/zhangnong/MJO/711_test/seasonal_cycle/ecmf/OLR/'+save_filename+'_seasonal_cycle_olr.npy', mean_data_1)
        print("save 1 success!!!")
        np.save('/WdHeDisk/users/zhangnong/MJO/711_test/seasonal_cycle/ecmf/OLR/'+save_filename+'_run_seasonal_cycle_olr.npy', mean_data_2)
        ...
        save_filename=str(filename)
        save_filename=save_filename[4:8]
        np.save('/WdHeDisk/users/zhangnong/MJO/711_test/seasonal_cycle/ecmf/OLR/'+save_filename+'_seasonal_cycle_olr.npy', mean_data)
