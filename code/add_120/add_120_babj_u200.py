...
#%matplotlib inline
#再分析数据olr
re_data=netCDF4.Dataset('/WdHeDisk/users/zhangnong/MJO/new_DATA/u200_1950-2022.nc')
re_data_olr=np.array(re_data.variables['u200'])
re_data_time=np.array(re_data.variables['time'])
#获取文件名
def get_files():
    for filepath,dirnames,filenames in os.walk(r'/WdHeDisk/users/zhangnong/MJO/711_test/s2s_data/babj/U200/'):
...
def read_data(filename):
    path='/WdHeDisk/users/zhangnong/MJO/711_test/s2s_data/babj/U200/'
    new_path='/WdHeDisk/users/zhangnong/MJO/711_test/add_120_re_data/babj/U200/'
    ...
    #构造新的nc文件
    da_olr=netCDF4.Dataset(new_path+filename[0:8]+'_add_120redata_babj_u200.nc','w',format='NETCDF4')
    ...
    #设置u200变量
    olr =da_olr.createVariable('u200','f',('time','latitude','longitude'))
    olr.units = 'm s**-1'
    #填充数据
    da_olr.variables['u200'][:,:,:]=fina_data_ttr[:,:,:]
    da_olr.close()
