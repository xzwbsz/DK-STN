...
def find_rmm_redata_s2s(filename):
    ...
    data=netCDF4.Dataset(path+filename+'_babj_olr.nc')
    ...
    #保存文件
    save_path='/WdHeDisk/users/zhangnong/MJO/711_test/re_RMM/babj/'
    if length==120:
        np.save(save_path+filename+'_babj_rmm.npy',fina_rmm)
    else:
        print('error!!!!')
        return
    print("save one!!!")
