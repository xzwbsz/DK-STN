...
#std: 5.437523725114618 1.9760929603145574 16.255560461342792
...
def cal_rmm(filename):
    ...
    print(type(filename))
    #根据文件名中的日期打开相对应的三个文件
    path1='/WdHeDisk/users/zhangnong/MJO/711_test/add_120_re_data/babj/OLR/'
    path2='/WdHeDisk/users/zhangnong/MJO/711_test/add_120_re_data/babj/U200/'
    path3='/WdHeDisk/users/zhangnong/MJO/711_test/add_120_re_data/babj/U850/'
    #读取数据
    data1=netCDF4.Dataset(path1+filename+'_add_120redata_babj_olr.nc')
    data2=netCDF4.Dataset(path2+filename+'_add_120redata_babj_u200.nc')
    data3=netCDF4.Dataset(path3+filename+'_add_120redata_babj_u850.nc')
    ...
    #一些变量
    path0='/WdHeDisk/users/zhangnong/MJO/711_test/s2s_data/babj/OLR/'
    data0=netCDF4.Dataset(path0+filename+'_babj_olr.nc')
    #时间开始
    ...
    print(length)
    save_path='/WdHeDisk/users/zhangnong/MJO/711_test/s2s_RMM/babj/'
    if length==120:
        np.save(save_path+filename[0:8]+'_babj_rmm.npy',true_RMM)
    else:
        print('error!!!!')
        return
for i in range (len(filenames)):
    str_filename=str(int(filenames[i]))
    cal_rmm(filename=filenames[i])
